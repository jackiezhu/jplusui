...
Utility.Path = typeof exports !== 'object' ? require('path') : {
    ...
    resolve: function () {
        ...
        for (var i = arguments.length - 1; i >= -1 && !resolvedAbsolute; i--) {
            var path = (i >= 0) ? arguments[i] : Utility.Path.basePath;
        ...
        // Normalize the path
        resolvedPath = Utility.Path._normalizeArray(resolvedPath.split('/').filter(function (p) {
            return !!p;
    ...
    normalize: function (path) {
        ...
        // Normalize the path
        path = Utility.Path._normalizeArray(path.split('/').filter(function (p) {
            return !!p;
        }), !isAbsolute).join('/');
    ...
    join: function () {
        ...
        return Utility.Path.normalize(paths.filter(function (p, index) {
            if (typeof p !== 'string') {
    ...
    relative: function (from, to) {
        ...
        from = Utility.Path.resolve(from);
        to = Utility.Path.resolve(to);
    ...
    dirname: function (path) {
        var result = Utility.Path._splitPath(path),
            root = result[0],
            dir = result[1];
    ...
    basename: function (path, ext) {
        var f = Utility.Path._splitPath(path)[2];
    ...
    extname: function (path) {
        return Utility.Path._splitPath(path)[3];
    }
 ...
 * 解析相对路径。
 */
Utility.resolveRelativePath = function (baseFilePath, relativePath, basePath) {

    baseFilePath = baseFilePath.replace(/\\/g, '/');
    ...
    if (relativePath.charAt(0) === '.') {
        ...
        basePath = basePath || ('/' + baseFilePath.replace(/[\?#].*$/, "")).replace(/\/[^\/]*$/, "").substr(1);
 ...
 * 读取文件内容。
 */
Utility.readFile = typeof exports === "object" ? function (filePath, success, error, encoding) {
    try {
        success && success(require("utilskit/io").readFile(filePath, encoding));
    } catch (e) {
        error && error(e);
    }
