...
ModuleBuilder.build = function (options) {
    ...
    ModuleBuilder.parseRefs(options, options.inputs, options.basePath, function (options) {
        ...
        function generateJsOrCss(options, jsOrCss) {
            ...
            options.log('正在生成 ' + (options[outputJsOrCss] || jsOrCss.toLowerCase()));
            ...
            if (options.minify) {
                ...
                if (outputJsOrCssMin) {
                    options.log('正在生成 ' + outputJsOrCssMin);
                } else {
                    options.log('正在压缩 ' + (options[outputJsOrCss] || jsOrCss.toLowerCase()));
                }
        ...
        case '.css':
            Utility.readFile(moduleFullPath, function (content) {
                ...
                ModuleBuilder.parseRefs(options, refs, modulePath, function (options) {
                    options[fileType === '.js' ? 'outputJsList' : 'outputCssList'][modulePath] = content;
                    callback(options);
                });

            }, options.error.bind(options), options.inputEncoding);
            break;
        case '':
